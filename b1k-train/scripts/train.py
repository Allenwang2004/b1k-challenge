...
def main(config: _config.TrainConfig):
    ...
    # Gather each image batch to host first: integer-indexing a sharded array (img[i]) triggers a
    # CUDA illegal-address fault on multi-GPU (fsdp_devices > 1) with jax 0.5.3 on this hardware.
    host_images = [jax.device_get(img) for img in batch[0].images.values()]
    images_to_log = [
        wandb.Image(np.concatenate([np.asarray(img[i]) for img in host_images], axis=1))
        for i in range(min(5, len(host_images[0])))
    ]
