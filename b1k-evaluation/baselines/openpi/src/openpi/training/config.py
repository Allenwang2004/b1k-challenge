...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_b1k_dense",   # 換個名字，避免跟原本的 pi05_b1k 撞名
        exp_name="openpi",
        project_name="B1K",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=50),  # 拿掉 paligemma_variant
        data=LeRobotB1KDataConfig(
            repo_id="behavior-1k/2025-challenge-demos",
            base_config=DataConfig(
                prompt_from_task=True,
        ...
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),
        num_train_steps=50_000,
        freeze_filter=pi0_config.Pi0Config(pi05=True, action_horizon=50).get_freeze_filter(),  # 同步拿掉
        ema_decay=None,
        val_log_interval=5000,
        val_repo_id="behavior-1k/2025-challenge-demos",
        val_episodes_index=list(range(190, 200)),
